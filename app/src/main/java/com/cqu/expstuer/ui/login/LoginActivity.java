 ...
 */
package com.cqu.expstuer.ui.login;
...
import com.cqu.expstuer.bean.Contact;
...
public class LoginActivity extends Activity implements View.OnClickListener {
    ...
    private void initData() {
        ...
        myTool.showInfo(MobSDK.getAppkey() + ", " + MobSDK.getAppSecret());
            ...
            public void afterEvent(int event, int result, Object data) {
                if (result == SMSSDK.RESULT_COMPLETE) {
                    ...
                    switch (reseaon) {
                        case REGISTER:
                            // 验证手机和国家后，注册用户到服务器
                            myTool.startActivityForResult(new Contact(phone, country), RegisterActivity.class, REQUESR_FOR_USERNAME);
                            break;
                        case RESETPWD:
                            // 验证手机和国家后，注册用户到服务器
                            myTool.startActivityForResult(new Contact(phone, country), ResetPwdActivity.class, REQUESR_FOR_USERNAME);
                            break;
                    }
                } else {
                    // TODO 处理错误的结果
    ...
    private void attempLogin() {
        ...
        myTool.log("url = " + myTool.getServAdd() + "loginCheck.do");
        OkHttpUtils.post()
                .url(myTool.getServAdd() + "loginCheck.do")
                    ...
                    public void onResponse(String response, int id) {
                        ...
                        try {
                            ...
                            switch (obj.getString("state")) {
                                ...
                                //登录出错
                                case "error":
                                    msg = obj.getString("error");//获取错误类型
                                ...
                                // 首次登录成功，获取token用于下次快捷登录
                                case "notokensuccess":
                                    //这里不使用break，他也是一种success
                                    String token = obj.getString("token");
                                    myTool.setToken(token);
                                    // 登录成功
                                case "success":
                                    // 更新信息
//                                    DataMaker.updateUserInfo(LoginActivity.this);
                                    msg = "登录成功";
...
//                                startService(new Intent(LoginActivity.this, TaskService.class));
                                    ...
                                    mLoginDialog.setTitleText("登录成功")
                                            .setContentText("您好" + nickName + "，学递来啦欢迎您！")
                                            .setConfirmClickListener(new SweetAlertDialog.OnSweetClickListener() {
                                                @Override
                                                public void onClick(SweetAlertDialog sweetAlertDialog) {
                                                    sweetAlertDialog.dismissWithAnimation();
...
//                                toLoginEm();//登录环信账号
    ...
    private void toLogin() {
        ...
        if ("".equals(edtUserName.getText().toString().trim())) {
            TextUtils.with(this).hintEdt(edtUserName, "请输入手机号码！");
            return;
        }

        if (edtUserName.getText().toString().trim().length() != 11) {
            TextUtils.with(this).hintEdt(edtUserName, "格式有误，11位手机号码！");
            return;
        }

        if (edtPassword.getText().toString().length() < 6) {
            TextUtils.with(this).hintEdt(edtPassword, "密码在6位以上！");
            return;
        }

        mLoginDialog = new SweetAlertDialog(this, SweetAlertDialog.PROGRESS_TYPE);
        mLoginDialog.setTitleText("正在登录···");
        mLoginDialog.show();
        attempLogin();
//        attempGetToken();
    }
    ...
    private void setTextListener() {
        ...
        TextUtils.with(this).restrictTextLenth(edtUserName, 11, "手机号最大长度不超过11!");
        TextUtils.with(this).restrictTextLenth(edtPassword, 12, "密码最大长度不超过12位!");
